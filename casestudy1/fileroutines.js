...
let getCountryJSONFromFSPromise = (file) =>
    new Promise((resolve, reject) => {
        fs.readFile(file, 'utf8', (err, data) => err ? reject(err) : resolve(JSON.parse(data)));
    });

let getAlertJSONFromFSPromise = (file) =>
    new Promise((resolve, reject) => {
        fs.readFile(file, 'utf8', (err, data) => err ? reject(err) : resolve(JSON.parse(data)));
    });

module.exports = {fileExistsPromise, getJSONFromWebPromise, writeJSONToFSPromise, getCountryJSONFromFSPromise, getAlertJSONFromFSPromise};
